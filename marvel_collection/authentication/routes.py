...
def signup():
    ...
    try:
        if request.method == 'POST' and form.validate_on_submit():
            ...
            flash(f'Hello, {name}.You have successfully created a user account: {email}', 'user-created')
    ...
    try:
        if request.method == 'POST' and form.validate_on_submit():
            ...
            if logged_user and check_password_hash(logged_user.password, password):
                ...
                flash(f'Hey There{logged_user.name}! You were successfully logged in.', 'auth-sucess')
                return redirect(url_for('site.home'))
            else:
